...
router.get("/:product",async (req,res) => {
    ...
    var item = await mongoose_api.productModel.findById(`${product}`, {});
    var all_deatiles = {id : `${product}`, product_name : `${item.product_name}`, price :`${item.price}`,picture_link : `${item.picture_link}` };
    res.send(all_deatiles);
})
